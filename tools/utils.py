...
def ltwh2midpoint(space):
    ...
    mid_pos[:, 1] = space['l'].to_numpy('float') + space['w'].to_numpy(
        dtype='float') / 2  #x_mid
    mid_pos[:, 2] = space['t'].to_numpy('float') + space['h'].to_numpy(
        dtype='float') / 2  #y_mid
    return mid_pos
    ...
    for i in range(int(num_ids)):
        ...
        if (point):
            for j in range(len(ordered_tracks['id_' + str(i + 1)])):
                ordered_tracks_point_class['id_' + str(i + 1) + '_' +
                                           str(j)] = Point(
                                               ordered_tracks['id_' +
                                                              str(i +
                                                                  1)][j, :])
...
def cos_ang_c(v1, v2):
    angle = (v1[:, 0] * v2[:, 0] + v1[:, 1] * v2[:, 1]) / (mod_c(v1) *
                                                           mod_c(v2))
    return angle
...
def initial_point(
    ids_list, o_t_i_a, vect_l, direction_x, direction_y
):  # determinacion de la distancia del punto inicial del vector a la proyeccion del punto inicial de la trayectoria en el vector de direccion.
...
def xy2vect(ids_list, xy_dict):  #determinacion del vector de vectores
    ...
    for i in ids_list:
        if (len(xy_dict['id_' + str(i)]) > 1):
            ordered_tracks_vect['id_' + str(
                i)] = -xy_dict['id_' + str(i)][:len(xy_dict['id_' + str(i)]) -
                                               1] + xy_dict['id_' + str(i)][1:]
            ordered_tracks_vect['id_' + str(i)][:, 2] = xy_dict['id_' +
                                                                str(i)][1:, 2]
        else:
...
def cos_ang_vect(
    ids_list, o_t_v, vect_l
):  #determinacion de vector de l, la segunda columna son datos fantasma
    ...
    for i in ids_list:
        ...
        ordered_tracks_vect_cos['id_' + str(i)] = cos_ang_c(
            o_t_v['id_' + str(i)][:, :2],
            vect_l * np.ones([
                np.shape(o_t_v['id_' + str(i)][:, :2])[0],
                np.shape(o_t_v['id_' + str(i)][:, :2])[1]
            ]))
        ordered_tracks_vect_cos['id_' + str(i)] = np.nan_to_num(
            ordered_tracks_vect_cos['id_' + str(i)])
    for i in ids_list:
        proyections['id_' +
                    str(i)][:, 0] = L['id_' +
                                      str(i)] * ordered_tracks_vect_cos['id_' +
                                                                        str(i)]
        proyections['id_' + str(i)][:, 0] = np.nan_to_num(
            proyections['id_' + str(i)][:, 0])
        proyections['id_' + str(i)][:, 1] = ordered_tracks_vect_cos['id_' +
                                                                    str(i)]
...
def space_t_diagram(list_init, init, list_vect,
                    vect):  #generacion del vector de l sumados
    space = {}
    for i in list_vect:
        space['id_' + str(i)] = np.zeros(
            [np.shape(vect['id_' + str(i)])[0], 2])
        for j in range(len(vect['id_' + str(i)])):
            if (j > 0):
                space['id_' +
                      str(i)][j][0] = vect['id_' + str(i)][j, 0] + np.sum(
                          vect['id_' + str(i)][:j, 0], axis=0)
...
def diagram_corrected(
    init_p, proyect
):  #generacion del vector de l sumados con la distancia al punto inicial
    diagram = init_p.copy()
    for i in proyect.keys():
        diagram[i] = np.zeros([np.shape(proyect[i])[0] + 1, 2])
        diagram[i][0, :] = init_p[i]
        diagram[i][1:, 0] = proyect[i][:, 0] + init_p[i][0] * np.ones(
            np.shape(proyect[i][:, 0]))
        diagram[i][1:, 1] = proyect[i][:, 1]
    return diagram


def gen_xy_in_line(
    first_p, last_p, space_time
):  #conversion del vector de l sumados a coordenadas en la recta del vector direccion
    ...
    for i in space_time.keys():
        ...
        if (not (isinstance(space_time[i], list))):
            xy[i][:, 0] = first_p[0] * np.ones([np.shape(
                xy[i])[0]]) + space_time[i][:, 0] * np.cos(gamma)
            xy[i][:, 1] = first_p[1] * np.ones([np.shape(
                xy[i])[0]]) + space_time[i][:, 0] * np.sin(gamma)
    ...
    if (method == 'great_circle'):
        for i in xy_prim.keys():
            ...
            for j in range(len(xy_prim[i])):
                L[i][j, 0] = great_circle(vect_first[:2],
                                          xy_prim[i][:, :3][j, :2]).km * 1e3
    if (method == 'geodesic'):
        for i in xy_prim.keys():
            L[i] = np.zeros([np.shape(xy_prim[i])[0], 2])
            L[i][:, 1] = xy_prim[i][:, 3]
            for j in range(len(xy_prim[i])):
                L[i][j, 0] = geodesic(vect_first[:2],
                                      xy_prim[i][:, :3][j, :2]).km * 1e3
    return L
...
def frm2time(L):
    ...
    for i in L.keys():
        ...
        for j in range(len(hour)):
            dts.append(
                dt.timedelta(hours=int(hour[j]),
                             minutes=int(minute[j]),
                             seconds=int(second[j]),
                             microseconds=int(np.floor(useg[j]))) +
                dt.datetime(2022, 2, 5, 5, 58, 48))
        time[i] = dts
    return time
...
def speeds(
    L_l
):  #determinacion del vector de velocidades y aceleraciones para un diccionario
    speed_ac_l = {}
    h = 1 / 60
    for i in L_l.keys():
        if (len(L_l[i]) > 30):
            speed_ac_l[i] = np.zeros([np.shape(L_l[i])[0] - 2, 3])
            speed_ac_l[i][:, 0] = ((L_l[i][:, 0] - np.roll(L_l[i][:, 0], 1)) /
                                   h)[2:]  # speed
            speed_ac_l[i][:,
                          1] = ((L_l[i][:, 0] - 2 * np.roll(L_l[i][:, 0], 1) +
                                 np.roll(L_l[i][:, 0], 2)) /
                                (h**2))[2:]  #acceleration
...
def spds_acce(
    x, y
):  #determinacion del vector de velocidades y aceleraciones para un vector
    h = 1 / 60
