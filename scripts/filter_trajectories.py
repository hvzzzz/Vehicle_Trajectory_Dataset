...
def main():
    ...
    #dir_trajs_raw = '../../vci-dataset-citr/data/trajectories/'
    #dir_trajs_filtered = '../../vci-dataset-citr/data/trajectories_filtered/'
    # params
    #fps = 29.97
    fps = 60
    dt = 1/fps
                    ...
                    if file[0] == 'p':
                        ...
                        for i, id_frame in enumerate(id_frames):
                            ...
                            if i == 0:  # initalize KF
                                ...
                                df_ped.loc[id_dataframe, 'vx_est']=(xy[:,0][i+1]-xy[:,0][i])/(1*dt)
                                #df_ped.loc[id_dataframe, 'vx_est'] = (float(df_ped[df_ped.frame == id_frame + 1].x)
                                #                                       - float(df_ped[df_ped.frame == id_frame].x)) / (
                                #                                                  1 * dt)
                                #print("if0")
                                #print((float(df_ped[df_ped.frame == id_frame + 1].x)-float(df_ped[df_ped.frame == id_frame].x)) / (1*dt))
                                #print((xy[:,0][i+1]-xy[:,0][i])/(1*dt))
                                ...
                                #                                                  1 * dt)
                                df_ped.loc[id_dataframe, 'vy_est']=(xy[:,1][i+1]-xy[:,1][i])/(1*dt)
                                #print((float(df_ped[df_ped.frame == id_frame + 1].y)-float(df_ped[df_ped.frame == id_frame].y)) / (1*dt))
                                #print((xy[:,1][i+1]-xy[:,1][i])/(1*dt))
